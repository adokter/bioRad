...
#'
#' By contrast, in \link[=summary.vp]{vplist} objects the profiles have no
#' time ordering, and can contain profiles of multiple radars.
#'
...
as.data.frame.vpts <- function(x, row.names = NULL, optional = FALSE,
  ...
  if (suntime) {
    ...
    sunrise <- suntime(x$dates, lat = lat, lon = lon, rise = TRUE)
    sunset <- suntime(x$dates, lat = lat, lon = lon, rise = FALSE)
