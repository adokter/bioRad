...
clean_mixture <- function(x, u, v, U, V, slow = 1, fast = 12, drop_slow_component = TRUE){
  ...
  if(mixture_airspeed > fast) warning("Airspeed of mixture exceeds airspeed of fast component")
  if(mixture_airspeed < slow) warning("Airspeed of slow component exceeds airspeed of mixture.")
  r <- slow/wind_speed
  p1 <- slow^2 - fast^2
