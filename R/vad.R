...
vad.pvol <- function(x, vp = NULL,..., range = NULL, height = NULL,
  ...
  if (!is.null(vp)) {
    vp_df <- as.data.frame(vp) |> dplyr::mutate(
      heightBin = glue::glue("{height}-{height + vp$attributes$where$interval} [m]"),
      heightBin = factor(heightBin, levels = heightBin)
    )
  ...
  if (!is.null(vp)) {
    ...
    vp_geom <- purrr::pmap(
      list(vp_df$ff[s], vp_df$dd[s], vp_df$height[s], vp_df$heightBin[s]),
      ~ ggplot2::geom_function(
        data = dplyr::bind_cols(data, data.frame(
          minBinHeight = ..3,
          maxBinHeight = ..3 + vp$attributes$where$interval,
          heightBin = ..4
        )),
  ...
  plt <- ggplot2::ggplot(data, ggplot2::aes(x = azim, y = VRADH)) +
