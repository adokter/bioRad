...
#' Distance from the radar over the earth's surface is calculated as \code{range*cos((beam elevation)*pi/180)}.
scan_to_spatial <- function(scan, k = 4 / 3, lat=NA, lon=NA, re = 6378, rp = 6357) {
  ...
  data=data.frame(azim=c(t(matrix(rep(seq(0,dim(scan)[3]-1)*ascale,dim(scan)[2]),nrow=dim(scan)[3]))),
                  ...
                  distance=rep(seq(1,dim(scan)[2])*rscale,dim(scan)[3])*cos(elev*pi/180))
...
#' @details uses \link{scan_to_spatial} to georeference the scan's pixels.
#' @export
#' @examples
...
scan_to_raster <- function(scan,nx=100,ny=100,xlim=NA,ylim=NA,crs="+proj=longlat +datum=WGS84",res=NA, k = 4 / 3, lat=NA, lon=NA, re = 6378, rp = 6357){
  ...
  # need to temporarily remove NA and NaN values, as raster::rasterize() does not accept such values
  NAN_TMP=99998
  NA_TMP=99999
