...
dbz_to_eta <- function(dbz, wavelength, K = 0.93) {
  ...
  assert_that(is.numeric(wavelength) & all(wavelength > 0), msg = "`wavelength` must be a strictly positive numeric.")
  assert_that(is.numeric(K) & all(K > 0), msg = "`K` must be a strictly positive numeric.")
...
eta_to_dbz <- function(eta, wavelength, K = 0.93) {
  assert_that(is.numeric(eta) & all(eta>0), msg = "`eta` must be a strictly positive numeric.")
  assert_that(is.numeric(wavelength) & all(wavelength > 0), msg = "`wavelength` must be a strictly positive numeric.")
  assert_that(is.numeric(K) & all(K > 0), msg = "`K` must be a strictly positive numeric.")
