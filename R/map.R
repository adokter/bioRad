...
map.ppi <- function(
  ...
  if (!missing(xlim)) {
    assertthat::assert_that(
      is.numeric(xlim),
      length(xlim) == 2,
      msg = "xlim must be a numeric vector of length 2"
    )
  }
  if (!missing(ylim)) {
    assertthat::assert_that(
  ...
  crs_missing <- if (inherits(x$data, "SpatRaster")) {
    !nzchar(terra::crs(x$data))
  } else {
    is.na(raster::crs(x$data))
  }
  if (crs_missing) {
    stop(
      "Not a projected ppi, map() expects a ppi generated by project_as_ppi() with argument project=TRUE"
    )
  }
  ...
  assert_valid_basemap <- function(basemap) {
    ...
    if (!(basemap %in% valid_types)) {
      stop(paste(
        "Invalid basemap argument. Must be one of:",
  ...
  if (!assertthat::are_equal(palette, NA)) {
    if (!(is.character(palette) && length(palette) > 1)) {
      stop("palette should be a character vector with hex color values")
    }
    # apply transparancy
    palette <- ggplot2::alpha(palette, alpha)
    n_color = length(palette)
    colorscale <- color_palette_to_scale_colour(
      param,
      zlim,
      palette,
  ...
  if (inherits(data, "SpatRaster")) {
    data <- methods::as(raster::brick(data), "SpatialGridDataFrame")
  }
  ...
  if (!missing(xlim)) {
    ...
    bbox_latlon_x <- sf::st_bbox(
      c(xmin = xlim[1], xmax = xlim[2], ymin = -90, ymax = 90),
      crs = 4326
    )
    bbox_projected_x <- sf::st_transform(
      sf::st_as_sfc(bbox_latlon_x),
      crs = 3857
    )
  ...
  if (!missing(ylim)) {
    ...
    bbox_latlon_y <- sf::st_bbox(
      c(xmin = -180, xmax = 180, ymin = ylim[1], ymax = ylim[2]),
      crs = 4326
    )
    bbox_projected_y <- sf::st_transform(
      sf::st_as_sfc(bbox_latlon_y),
      crs = 3857
    )
    projected_ylim <- sf::st_bbox(bbox_projected_y)[c("ymin", "ymax")]
  }
  if (is.null(radar_size)) {
    radar_geom <- list()
  } else {
    radar_geom <- list(ggplot2::geom_point(
      data = radar_df,
      ggplot2::aes(x = X, y = Y),
      shape = 21,
      fill = "transparent",
