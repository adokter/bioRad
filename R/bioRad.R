...
#' @examples
#' VPFileQ("this/is/not/a/valid/VP/file.h5")   #> FALSE
#'
...
plot.VP=function(vpb, xlab="density [#/km^3]",ylab="height [km]",main="Vertical profile", type="l",line.col='red',line.lwd=1,...){
  plot(vpb$profile$dens,vpb$profile$HGHT/1000, xlab=xlab,ylab=ylab,main=main,...)
  points(vpb$profile$dens,vpb$profile$HGHT/1000, col=line.col,lwd=line.lwd,type="l")
}
...
#' @param radar string containing a unique radar identifier
...
vol2bird =  function(volume.in, profile.out="", volume.out="",verbose=F){
  ...
  system(paste("docker run -v ",filedir,":/data -d --name vol2bird adokter/vol2bird sleep infinity",sep=""),ignore.stdout=T)
...
#'
#' Checks that \href{https://www.docker.com/}{Docker} is running correctly
#' @export
