...
#' Stored hdf5 profiles can be read from disk with \link{read_vpfiles} (for a single file) or with
#' \link{readvp.list} (for a list of files)
#'
#' For users running their own installation of vol2bird outside R and Docker, the function \link{\code{read_vpts}}
...
#' \item \code{VPTable.txt}: example standard output of \link{calculate_vp} piped to a text file. Locate this file in your local installation with \code{system.file("extdata", "VPtable.txt", package="bioRad")}. Read it with \link{\code{read_vpts}}.
#' }
#' }
#'
#' @references
