...
nexrad_to_odim_tempfile <- function(pvolfile_nexrad, verbose = FALSE,
                                    mount = dirname(pvolfile_nexrad)) {
  ...
  pvolfile_nexrad_docker <- paste(prefix, basename(pvolfile_nexrad), sep = "")
  ...
  if (.Platform$OS.type == "unix") {
    result <- system(
      paste("docker exec vol2bird bash -c 'cd data && rsl2odim ",
            pvolfile_nexrad_docker, vol_tmp_docker, "'"), ignore.stdout = !verbose)
  } else {
