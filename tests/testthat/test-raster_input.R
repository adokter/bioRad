...
test_that('integrate to ppi produces simular output',{
		  ...
		  expect_equal(raster(integrate_to_ppi(example_pvol, example_vp, raster=raster(my_ppi$data))$data),
			       raster(my_ppi$data))
})
...
test_that('integrate to ppi produces same values on cropped raster',{
  ...
  expect_equal(values(raster(integrate_to_ppi(example_pvol, example_vp,
					      raster=raster::crop(raster(my_ppi$data), extent(0,3000,1000,5000)))$data)),
...
test_that('check if other projection gives same result',{
  ...
  expect_equal(values(raster(integrate_to_ppi(example_pvol, example_vp, raster=my_raster)$data))[!is.na(values(my_raster))],
