...
test_that(
  "as.data.frame(): by default, suntime is TRUE and suntime information is added to each row",
  {
    data("example_vp")
...
test_that(
  "as.data.frame(): if suntime is (explicitly) TRUE, suntime information is added to each row",
  {
    data("example_vp")
...
test_that("as.data.frame(): values in suntime/sunset/day cols are correct", {
  ...
  expected_sunrise <-
    with_tz(
      as.POSIXlt("2015-10-18 07:45:26", tz = "Europe/Stockholm"),
      "UTC"
    )
  expected_sunset <-
    with_tz(
      as.POSIXlt("2015-10-18 18:01:13", tz = "Europe/Stockholm"),
      "UTC"
    )
  ...
  expected_sunrise <-
    with_tz(
      as.POSIXlt("2015-10-18 08:09:09", tz = "Europe/Brussels"),
      "UTC"
    )
  expected_sunset <-
    with_tz(
      as.POSIXlt("2015-10-18 18:45:35", tz = "Europe/Brussels"),
      "UTC"
    )
  expect_equal(as.POSIXlt(df$sunrise[1]), expected_sunrise, tolerance = 5) # Apparently, tolerance is expressed in minutes
  expect_equal(as.POSIXlt(df$sunset[1]), expected_sunset, tolerance = 5)
...
test_that(
  "as.data.frame(): by default, geo is TRUE and lat/lon/antenna height is added to each row",
  {
    data("example_vp")
...
test_that(
  "as.data.frame(): if geo is (explicitly) TRUE, lat/lon/antenna height is added to each row",
  {
    data("example_vp")
...
test_that(
  "as.data.frame(): if we manually set lat/lon, those are the value that should appear in the df",
  {
    data("example_vp")

    df <-
      as.data.frame(example_vp,
        geo = TRUE,
        lat = 50.6472,
        lon = 4.3603
      )
...
test_that("as.data.frame(): users can manually choose included quantities", {
  ...
  expected_message <-
    "azertyuiop not an available quantity, select one or more of ff,dbz,dens,u,v,gap,w,n_dbz,dd,n,DBZH,height,n_dbz_all,eta,sd_vvp,n_all"
