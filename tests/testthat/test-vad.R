...
test_that("vad() errors on incorrect parameters", {
  expect_error(
    vad(pvol, velocity_quantity = 1:2),
    "velocity_quantity is not a character vector"
  )
  expect_error(
    vad(pvol, velocity_quantity = c("VRAD", "VRADV")),
    "None of the specified velocity quantities could be found in the polar volume data"
  )

  expect_error(
    vad(vp),
    "no applicable method for 'vad' applied to an object of class \"vp\""
  )
  ...
  expect_error(
    vad(pvol, alt_max = "a"),
    "alt_max is not a numeric or integer vector"
  )
  expect_error(
    vad(pvol, alt_max = 1:2),
    "alt_max is not NULL or rlang::is_scalar_vector(x = alt_max) is not TRUE",
    fixed = TRUE
  )
  expect_error(
    vad(pvol, alt_min = "a"),
    "alt_min is not a numeric or integer vector"
  )
  expect_error(
    vad(pvol, alt_min = 1:2),
    "alt_min is not NULL or rlang::is_scalar_vector(x = alt_min) is not TRUE",
    fixed = TRUE
  )
  expect_error(
...
test_that("plot from vad() matches some expectations", {
  ...
  expect_equal(
    plt$mapping,
    ggplot2::aes(x = azim, y = VRADH),
    ignore_attr = TRUE
  )
  expect_length(
    plt$data$scan_nr |> unique(),
    sum(get_elevation_angles(pvol) < 2)
  )
})
...
test_that("vad raises cosine correction warnings and is applied", {
  expect_warning(
    plt_rg <- vad(
      pvol,
      cosine_correction = "range_gates",
      range_min = 5000,
      range_max = 25000
    ),
    'There are data that have a relatively low nyquist velocity'
  )
  expect_warning(
    plt_vp <- vad(pvol, vp, cosine_correction = "vp"),
    'The data to plot is based on multiple elevation angle'
  )
  expect_equal(
    plt_rg$data$VRADH,
    plt_vp$data$VRADH * 1 / cospi(plt_vp$data$where.elangle / 180)
  )
  expect_identical(
    plt_vp$layers[[2]]$stat_params$args$v,
    vp$data$ff[2] * cospi(mean(get_elevation_angles(pvol)) / 180)
  )
  expect_identical(plt_vp$layers[[3]]$stat_params$args$a, vp$data$dd[3])
  pvol_tmp <- pvol
  pvol_tmp$scans[[3]]$attributes$where$elangle <- 20
  expect_warning(
    plt_none <- vad(pvol_tmp, vp),
    "Data with relatively large elevation angles are included."
  )
  ...
  expect_identical(
    plt_scn$layers[[2]]$stat_params$args$v,
    vp$data$ff[2] * cospi((get_elevation_angles(scan)) / 180)
  )
})
