...
test_that("filter_vpts() returns error on incorrect parameters", {
  ...
  expect_error(filter_vpts(vpts, min = "2016-09-01 00:02:00", max = "2016-09-10 11:56:00", night = "not_a_boolean"))
})
...
test_that("filter_vpts() returns the correct class of object", {
  expect_equal(class(filter_vpts(vpts, min = "2016-09-03 00:02:00")), class(vpts))
...
test_that("filter_vpts() returns only night or only day correctly", {
  night_vpts <-filter_vpts(vpts, min = "2016-09-04 05:00:00", max = "2016-09-04 12:00:00", night = TRUE)
  day_vpts <-filter_vpts(vpts, min = "2016-09-04 05:00:00", max = "2016-09-04 12:00:00", night = FALSE)

  expect_true(unique(bioRad::check_night(night_vpts$datetime,
                                  night_vpts$attributes$where$lon,
                                  night_vpts$attributes$where$lat)))

  expect_false(unique(bioRad::check_night(day_vpts$datetime,
