...
* select the drives on which you will be processing radar files
* click apply
...
install.packages("fields9
install.packages("devtools")
```

### 4. bioRad 
You are now ready to install the bioRad package. In R first load the devtools package, then install:
```
library(devtools)
install_github("adokter/bioRad")
```

### installation note: ggplot2 and ggmap on Mac OSX
...
### installation note: rgdal on Mac OSX / linux:
