---
title: "Range bias correction with bioRad"
output:
...
## 1. preparations
First install bioRad from Github from the rangebias branch:
```{r,eval=FALSE}
install.packages("devtools")
library(devtools)
install_github("adokter/bioRad@rangebias")
```
Load bioRad
```{r}
library(bioRad)
...
## 4. range bias correction
Range bias correction is implemented as follows:
...
For each pixel on the ground, we thus end up with a set of $eta_{expected}$ and a set of $eta_{observed}$. From those we can calculate a correction factor at that pixel, as $correction\_factor=\sum{eta_{observed}}/\sum{eta_{expected}}$, with the sum running over scans.
...
- the spatial range-bias corrected PPI for `vid` is simply the correction_factor image, multiplied by the `vid` calculated for the profile
- Likewise, the spatial range-bias corrected PPI for vir is simply the `correction_factor` image, multiplied by the `vir` calculated for the profile

## 5. range bias correction example
...
my_corrected_ppi <- project_as_corrected_ppi(example_pvol,example_vp, res=500,xlim=c(-100000,100000),ylim=c(-100000,100000))
my_corrected_ppi
```

The range corrected PPI has four parameters: vir, vid, correction_factor, overlap. Let's plot the correction factor:
```{r}
# plot the correction_factor:
plot(my_corrected_ppi,param="correction_factor")
```
...
At large distances from the radar we expect correction for range bias effects to become more difficult:
...
bpo=beam_profile_overlap(example_pvol, example_vp, seq(0,100000,1000), quantity='eta')
...
The range-bias corrected PPI also contains a field `overlap`, which contains the same metric but then calculated on the grid of the PPI:
```{r}
...
- double-checking all the naming conventions
- writing more detailed documentation of the functions
- a more textual description of the method, as basis for a paper.
